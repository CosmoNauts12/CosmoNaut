...
export default function WorkspaceSidebar({ onSelectRequest }: { onSelectRequest?: (request: SavedRequest & { collectionId: string }) => void }) {
  ...
  const {
    ...
    renameRequest,
    createWorkspace,
    deleteWorkspace: _deleteWorkspace, // Not used in UI yet
    renameWorkspace,
    clearHistory
  } = useCollections();
   ...
   */
  const openPrompt = (title: string, defaultValue: string, onSubmit: (val: string) => void) => {
    setPromptModal({ isOpen: true, title, defaultValue, onSubmit });
  };

  /**
   * Opens the confirmation modal.
   */
  const openConfirm = (title: string, message: string, onConfirm: () => void, isDestructive = false, confirmLabel = "Confirm") => {
    setConfirmModal({ isOpen: true, title, message, onConfirm, isDestructive, confirmLabel });
  };

  const handleCreateWorkspace = () => {
    openPrompt("New Workspace", "", async (name) => {
  ...
  const handleRenameWorkspace = () => {
    ...
    openPrompt("Rename Workspace", activeWorkspace.name, async (name) => {
      if (name) await renameWorkspace(activeWorkspace.id, name);
    });
  };

  const handleCreateCollection = () => {
    openPrompt("New Collection", "", async (name) => {
      if (name) await createCollection(name);
    });
  };

  const handleRenameCollection = (id: string, currentName: string) => {
    openPrompt("Rename Collection", currentName, async (name) => {
      if (name) await renameCollection(id, name);
    });
  };

  const handleRenameRequest = (id: string, collectionId: string, currentName: string) => {
    openPrompt("Rename Request", currentName, async (name) => {
      if (name) await renameRequest(id, collectionId, name);
    });
  };
  ...
  const handleDeleteRequest = (requestId: string, collectionId: string, name: string) => {
    if (settings.confirmDelete) {
      openConfirm(
        ...
        async () => await deleteRequest(requestId, collectionId),
        true,
        "Delete"
      );
    } else {
  ...
  const handleDeleteCollection = (collectionId: string, name: string) => {
    if (settings.confirmDelete) {
      openConfirm(
        "Delete Collection",
        `Are you sure you want to delete collection "${name}" and all its requests?`,
        async () => await deleteCollection(collectionId),
        true,
        "Delete"
      );
    } else {
