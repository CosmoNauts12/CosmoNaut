        ...
        <div className="relative flex items-center h-full">
          ...
          {dropdownOpen && (
            <>
              ...
              />
              <div className="absolute right-0 top-12 w-64 rounded-2xl border border-card-border bg-white dark:bg-[#0a1f2e] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 origin-top-right duration-200 z-50">
                {/* Solid Gradient Overlays (No Transparency) */}
                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/10 to-blue-500/10 dark:from-emerald-900 dark:to-[#0a2e24] pointer-events-none" />
                <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-emerald-500/10 dark:from-emerald-800/20 to-transparent pointer-events-none" />
                ...
                <div className="relative z-10 p-5">
                    ...
                    <div className="mt-2">
                      {isEditingStatus ? (
                        <input
                          ...
                          className="w-full bg-foreground/5 border border-emerald-500/30 dark:border-primary/30 rounded-lg px-2 py-1.5 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/10 dark:focus:ring-primary/10 transition-all"
                        />
                      ) : (
                        <div
                          onClick={() => setIsEditingStatus(true)}
                          className={`mt-1 flex items-center gap-2 px-3 py-2 rounded-xl border transition-all group active:scale-[0.98] ${settings.status
                            ? 'bg-emerald-500/5 border-emerald-500/20 text-foreground'
                            : 'bg-foreground/[0.02] border-card-border text-muted hover:text-emerald-600 dark:hover:text-primary hover:border-emerald-500/30 dark:hover:border-primary/30'
                            }`}
                        >
                          <span className={`${settings.status ? 'text-emerald-500 dark:text-primary animate-pulse' : 'opacity-50'} transition-transform group-hover:scale-110`}>{settings.status ? '🟢' : '✨'}</span>
                  ...
                  <div className="space-y-1">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-muted hover:text-emerald-600 dark:hover:text-primary hover:bg-emerald-500/5 dark:hover:bg-primary/5 transition-all"
                    >
                      Profile Settings
                    </button>
                    <button className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-muted hover:text-emerald-600 dark:hover:text-primary hover:bg-emerald-500/5 dark:hover:bg-primary/5 transition-all">
                      Resource Center
                    </button>
                  </div>
